...
from bool_parser import BooleanFunctionParser, ParseError, tokenize, shunting_yard, evaluate_rpn
...
class TestTokenize(unittest.TestCase):
    ...
    def test_and(self):
        tokens = tokenize("a&b")
        self.assertEqual(tokens, [(TOKEN_VARIABLE, 'a'), (TOKEN_OPERATOR, '&'), (TOKEN_VARIABLE, 'b')])
    ...
    def test_parentheses(self):
        tokens = tokenize("(a&b)")
        self.assertEqual(tokens, [(TOKEN_LPAREN, '('), (TOKEN_VARIABLE, 'a'), (TOKEN_OPERATOR, '&'), (TOKEN_VARIABLE, 'b'), (TOKEN_RPAREN, ')')])
...
class TestBooleanFunctionParser(unittest.TestCase):
    ...
    def test_xor(self):
        ...
        self.assertEqual(parser.evaluate({'a': 0, 'b': 0}), 0)
        self.assertEqual(parser.evaluate({'a': 1, 'b': 0}), 1)
        self.assertEqual(parser.evaluate({'a': 1, 'b': 1}), 0)
